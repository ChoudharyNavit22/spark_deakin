...
var createUser = function (payloadData,callback) {
    ...
    async.series([
        ...
        function (cb) {
            ...
            Service.LevelService.createLevel(leveldata,function(err,dataFromDB){
                if(err){
                    cb(err)
                }
                else{
...
var generateQuestion = function(userData,callback){
    var customerData;
    var customerLevel;
    var result_ques;
    async.series([
...
function LevelQuestion(level){
    ...
    if(level == 1){
        ...
        x = (y + Math.floor(Math.random() * y) + 1);
        ans = x % y;
        quiz = {
            'ques': x +" mod "+ y+" ?",
            'ans' : ans
...
function updateLevelFunc(level,correct){
    var delta = 0;
    var new_level = level;
    if(correct == 1){
        delta = 1/3;
